...
class ZSN2NTrainer:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.is_ss:
            noisy = ss(noisy,self.n_fft,self.hop_length).to(self.device)
        ...
        dg1,dg2 = subsample(denoised,self.subsample_k)
        ...
        # loss = (loss_res + loss_cons) #+ 0*
        loss = (loss_res_time + loss_cons_time)
        # loss = loss_res
    ...
    def predict_step(self, batch, batch_idx):
        ...
        if self.is_ss:
            noisy = ss(noisy,self.n_fft,self.hop_length).to(self.device)
        pred_clean = noisy - self(noisy)
        pred_noise = self(noisy)
        # window = torch.hann_window(self.n_fft,device=noisy.device)
        # noisy_stft = torch.stft(noisy.squeeze(0), n_fft=self.n_fft, hop_length=self.hop_length, return_complex=True,window=window)
        # noisy_stft = torch.view_as_real(noisy_stft)
